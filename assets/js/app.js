...
/* Upload source override start */

/*
  Makes upload menu items work without changing old app.js handlers.
  It uses capture mode, so it catches the click before old alert handlers.
*/

document.addEventListener('click', event => {
  const button = event.target.closest?.('[data-upload-option]');
  if (!button) return;
  ...
  handleUploadSourceOverride(button.dataset.uploadOption || '', button.textContent.trim());
}, true);

function handleUploadSourceOverride(option, label = '') {
  ...
  if (!fileInput) {
    sourceToastOverride('File input not found.');
    return;
  }

  if (normalized.includes('computer') || normalized === 'local') {
    fileInput.click();
    return;
  }

  if (normalized.includes('url')) {
    importFileFromUrlOverride();
    return;
  }
  ...
  if (
    ...
    const cleanLabel = label.replace(/^From\s+/i, '').trim() || 'cloud storage';
    sourceToastOverride(`Choose a ${cleanLabel} file from your synced folder.`);
    fileInput.click();
    return;
  }
...
async function importFileFromUrlOverride() {
  const rawUrl = window.prompt('Paste a direct image URL');
  ...
  try {
    ...
    sourceToastOverride('Invalid URL.');
    return;
  }

  try {
    sourceToastOverride('Importing URL...');
    ...
    if (!isSupportedImageBlobOverride(blob, url)) {
      throw new Error('Unsupported file type.');
    }

    const fileName = fileNameFromUrlOverride(url, blob);
    const file = new File([blob], fileName, {
      type: blob.type || mimeFromNameOverride(fileName) || 'application/octet-stream'
    });

    handleFiles([file]);
    sourceToastOverride('URL file added.');
  } catch (error) {
    console.error(error);
    sourceToastOverride('URL import failed. Use a direct image link.');
  }
}

function isSupportedImageBlobOverride(blob, url) {
  const mime = String(blob.type || '').toLowerCase();

  if (mime.startsWith('image/')) return true;

  return /\.(png|jpe?g|webp|avif|gif|svg|bmp|ico)$/i.test(url.pathname);
}

function fileNameFromUrlOverride(url, blob) {
  ...
  if (cleanName && /\.[a-z0-9]+$/i.test(cleanName)) {
    return cleanName;
  }

  const ext = extensionFromMimeOverride(blob.type) || 'png';
  const base = cleanName || 'url-image';

  return `${base}.${ext}`;
}

function extensionFromMimeOverride(mime = '') {
  const map = {
    'image/png': 'png',
    'image/jpeg': 'jpg',
    'image/webp': 'webp',
...
function mimeFromNameOverride(name = '') {
...
function sourceToastOverride(message) {
  if (typeof notifyFormatLock === 'function') {
    notifyFormatLock(message);
    return;
  }
